...
let myId = null,
    ...
    clientVersion = '???????';
let modules = {},
    refitRates = { remove: 0.5, rotate: 0.25, stops: 32 };
...
const installsOf = (h) =>
    Object.fromEntries(
        ((hulls[h] || {}).installs || []).map((p) => [p.id, p.at]),
    );
...
const minZoom = () =>
    Math.hypot(canvas.width / dpr(), canvas.height / dpr()) / 2 / maxView;
...
const driftList = (kind, clock) =>
    [...moving[kind].values()].map((e) => stateAt(e, clock));
...
ws.onmessage = (e) => {
    ...
    if (m.t === 'walls') {
        for (const [k, rings, mat] of m.add)
            walls.set(k, { ...withBox(rings), mat });
    ...
    for (const [kind, rows] of Object.entries(m.a || {})) {
        ...
        for (const row of rows) {
            ...
            moving[kind].set(id, {
                ...
                of,
                track,
                prev: had ? had.track : null,
                b0: performance.now() - RENDER_DELAY,
            });
        }
    }
...
canvas.addEventListener('pointerdown', (e) => {
    ...
    if (pointers.size === 1) {
        ...
        const mk = markHit(w);
        if (mk && selection.size) {
            pressedControl = { kind: 'mark', key: mk.k };
            return;
        }
        const ctl = paneHit(w);
        if (ctl) {
        ...
        if (!rotating) {
            ...
            if (s || selection.size) {
                ...
                haptic(
                    holdPulse(
                        s ?
                            !(selection.has(s.id) && selection.size > 1)
                        :   false,
                    ),
                );
...
function haptic(pattern) {
    if (!vib.ok) return;
    try {
        vib.last = navigator.vibrate(pattern);
        vib.calls++;
    } catch {
        vib.last = 'threw';
    }
...
const PULSE_ADD = 25,
    PULSE_DROP = 12; // one long tick to add, two short to remove
const holdPulse = (adding) =>
    adding ?
...
function focusTargets() {
    ...
    return [...ids]
        .map((id) => allShips.find((s) => s.id === id))
        .filter(Boolean);
}
...
function orderMove(world) {
    ...
    if (!formed) {
        ...
        cy /= picked.length;
        for (const s of picked)
            formation.set(s.id, { dx: s.x - cx, dy: s.y - cy });
    }
    for (const s of picked) {
...
const GROUP_PAD = 90,
    GROUP_MIN_R = 130,
    CLEAR_ANGLE = -Math.PI * 0.75; // up and to the left
...
function gunRows(ship) {
    ...
    sides.forEach((side, c) =>
        side.forEach((i, r) => list.push({ i, label: name[c] + (r + 1) })),
    );
...
function syncDetails() {
    ...
    if (!ships.some((s) => s.id === openShip))
        openShip =
            ships.some((s) => s.id === designated) ? designated : (
                (ships[0]?.id ?? null)
            );
    ...
    const key =
        ...
        `|${openShip}|${designated}|${openTab}` +
        `|${(open?.ft || []).map((f) => f[1]).join(',')}` +
        `|${Object.entries(open?.hold || {})
            .filter(([, n]) => n > 0)
            .join(',')}`;
    if (key !== builtKey) {
        builtKey = key;
    ...
    for (const s of ships) {
        ...
        if (s.id === openShip) {
            ...
            if (PANELS[openTab].cargo) {
                ...
                const carried = Object.entries(s.hold || {}).filter(
                    ([, n]) => n > 0,
                );
            ...
            if (PANELS[openTab].guns && (s.ft || []).length)
                body.append(gunGrid(s));
...
const rotSame = (a, b) =>
    Math.abs(Math.atan2(Math.sin(a - b), Math.cos(a - b))) < 0.02;
...
function refitItems(was, fit) {
    ...
    for (const id of new Set([...before.keys(), ...after.keys()])) {
        const a = before.get(id),
            b = after.get(id);
        if (a && (!b || b.type !== a.type))
            put('Remove', a.type, price(a.type) * refitRates.remove);
...
function refitTargetAt(fit, where, held, p, type = null) {
    ...
    const ok =
        !rest.some((f) => f.install === best) &&
        !refitClash(rest, where, best, what);
    return { id: best, ok };
}
        ...
        g.append(
            svgEl('circle', {
                ...
                fill:
                    ...
                    :   'rgba(108,168,255,.16)',
                stroke:
                    blocking ? 'rgba(255,107,138,.6)' : 'rgba(108,168,255,.5)',
                'stroke-width': 0.7,
            }),
        );
...
function drawRefit() {
    ...
    if (refitShowItems) {
        ...
        const lines = [];
        for (const [kind, byType] of Object.entries(
            refitItems(refitting.was, refitting.fit),
        ))
            for (const [type, cell] of Object.entries(byType))
                lines.push(
                    `${kind} ${cell.n} ${modTitle(type)}${cell.n === 1 ? '' : 's'}` +
    ...
    svg.append(
        svgEl('path', {
            d: `M${DECK[0][0]} ${DECK[0][1]}L${DECK[1][0]} ${DECK[1][1]}`,
            stroke: '#24384f',
            'stroke-width': 0.8,
        }),
    );
    ...
    // is a guess.
    const lift =
        refitDrag && !refitDrag.ring && refitDrag.moved && refitDrag.at ?
            refitDrag
        :   null;
    ...
    const onto =
        carried ? refitTargetAt(refitting.fit, where, carried, lift.at) : null;
    // A point refused because something is in the way -- rather than because it is taken --
    // says what is in the way.
    const blocked = new Set(
        onto && !onto.ok && !refitting.fit.some((f) => f.install === onto.id) ?
            refitBlockers(
                refitting.fit,
                where,
                carried,
    ...
    for (const p of (hulls[s.h] || {}).installs || []) {
        ...
        if (!held) {
            const free =
                !refitting.pick ||
                !refitClash(refitting.fit, where, p.id, refitting.pick);
            ...
            if (refitting.sel === p.id && aims(held.type)) {
                ...
                const hx = Math.cos(held.rot) * 17,
                    hy = Math.sin(held.rot) * 17;
                g.append(
                    svgEl('circle', {
                        cx: hx,
                        cy: hy,
                ...
                const arc = svgEl('path', {
                    ...
                    'stroke-width': 1,
                    d:
                        `M${Math.cos(held.rot - mod.arcHalf) * 26} ${Math.sin(held.rot - mod.arcHalf) * 26}` +
            ...
            g.append(
                moduleIcon(held, refitting.sel === p.id, blocked.has(p.id)),
            );
        }
        svg.append(g);
    }
    ...
    if (carried) {
        const ghost = svgEl('g', {
            transform: `translate(${lift.at.x} ${lift.at.y})`,
            opacity: 0.95,
        });
    ...
    const kinds = Object.keys(shown).filter(
        (t) => shown[t] > 0 && !(modules[t] || {}).fixed,
    );
    if (!kinds.length) {
    ...
    for (const t of kinds) {
        ...
        b.dataset.type = t;
        b.innerHTML =
            `${modName(t)} <span class="n">×${ghost ? 1 : stock[t]}</span>` +
...
refitEl.addEventListener('pointerdown', (e) => {
    ...
    const install =
        ring || target.closest('[data-install]').getAttribute('data-install');
...
refitEl.addEventListener('pointerup', (e) => {
    ...
    if (drag.type !== undefined) {
        if (drag.moved) {
            ...
            const onto =
                drag.at &&
                refitTargetAt(refitting.fit, where, null, drag.at, drag.type);
            if (onto && onto.ok) {
                refitting.fit.push({
                    install: onto.id,
                    type: drag.type,
    ...
    if (!drag.moved) {
        ...
        if (!held && refitting.pick) {
            if (!refitClash(refitting.fit, where, drag.install, refitting.pick))
                refitting.fit.push({
                    install: drag.install,
                    type: refitting.pick,
                    rot: 0,
                });
        } else if (held) {
            refitting.sel =
                refitting.sel === drag.install ? null : drag.install;
            refitting.pick = null;
        }
        drawRefit();
        return;
    ...
    const onto =
        p ?
            refitTargetAt(refitting.fit, where, held, p)
        :   { id: drag.install, ok: true };
    if (!onto) {
...
function svgPoint(e) {
    ...
    return {
        x: vb.x + (e.clientX - r.left - (r.width - vb.width * k) / 2) / k,
...
function gunGrid(s) {
    ...
    for (const { i, label } of list) {
        ...
        cell.className = 'gun';
        cell.innerHTML =
            `<div class="gl">${moduleSvg(type, 12)}${label}${WRENCH}</div>` +
...
const ENV_W = 240,
    ...
    ENV_PAD = 9;
const ENV_GRAB = 11,
    ENV_KILL = 20; // svg units, ~14 and ~25 screen px
...
function envelope(shipId, kind, label, panel, initial) {
    ...
    const IW = ENV_W - 2 * ENV_PAD,
        IH = ENV_H - 2 * ENV_PAD;
    const X = (x) => ENV_PAD + warp(x) * IW,
        Y = (y) => ENV_PAD + (1 - y / 100) * IH;
    const xOf = (px) => unwarp(Math.max(0, Math.min(1, (px - ENV_PAD) / IW)));
    const yOf = (py) =>
        Math.max(0, Math.min(100, (1 - (py - ENV_PAD) / IH) * 100));
    ...
    el.className = 'env';
    el.innerHTML =
        `<div class="envhead">${label}<b></b></div>` +
    ...
    const paint = () => {
        ...
        curve.setAttribute(
            'points',
            drawn.map(([x, y]) => `${X(x)},${Y(y)}`).join(' '),
        );
        stops.textContent = '';
        pts.forEach(([x, y], i) => {
            const c = document.createElementNS(SVG_NS, 'circle');
            c.setAttribute('cx', X(x));
            c.setAttribute('cy', Y(y));
    ...
    svg.addEventListener('pointermove', (e) => {
        ...
        doomed =
            (
                !end &&
                (py < ENV_PAD - ENV_KILL || py > ENV_H - ENV_PAD + ENV_KILL)
            ) ?
                held
            :   -1;
        paint();
...
function freeRect() {
    ...
    if (b.height >= ch - 2) r.x1 = Math.max(0, b.left);
    else r.y1 = Math.max(0, b.top);
    return r;
}
...
function buildPane() {
    ...
    if (cmdShip) {
        ...
        list.push({
            ...
            cx: a.x,
            cy: a.y,
            track: CONTROL_R / cam.zoom,
            angle: dragHeading ?? cmdShip.hd,
            r: GRAB_R,
            pinned: true,
        });
    }
...
const paneSpot = (c) => ({
    x: c.cx + Math.cos(c.angle) * c.track,
    y: c.cy + Math.sin(c.angle) * c.track,
});
...
function resolvePane(list) {
    ...
    for (const c of list) {
        ...
        const blocked = (p) => {
            const sx = cw / 2 + (p.x - cam.x) * cam.zoom,
                sy = ch / 2 + (p.y - cam.y) * cam.zoom;
            return (
                sx < free.x0 + pad ||
                sx > free.x1 - pad ||
                sy < free.y0 + pad ||
                ...
                list.some(
                    (o) =>
                        o !== c &&
                        o.pos &&
                        Math.hypot(o.pos.x - p.x, o.pos.y - p.y) <
                            (o.r + c.r + CONTROL_PAD) / cam.zoom,
                )
            );
        };
...
function paneHit(world) {
    for (const c of pane)
        if (Math.hypot(world.x - c.pos.x, world.y - c.pos.y) < c.r / cam.zoom)
            return c;
    return null;
}
...
function sendFace(a, force) {
    ...
    lastFaceSend = now;
    if (ws.readyState === 1 && designated !== null)
        ws.send(JSON.stringify({ t: 'face', ship: designated, a }));
}
...
function release(e) {
    ...
    if (pressedControl) {
        ...
        if (dragged < 8 && ctl.kind === 'focus') {
            ...
            orderFocus(
                null,
                [...selection].filter(
                    (id) => fleet.find((s) => s.id === id)?.fo === ctl.target,
                ),
            );
        }
        return;
    ...
    if (longFired) {
        longFired = false;
        return;
    } // the hold already acted; the release is not a tap
    if (
        dragged < 5 &&
        performance.now() - pressedAt < 400 &&
        ws.readyState === 1
    ) {
        const r = canvas.getBoundingClientRect();
        ...
        const hit = shipAt(p);
        if (e.shiftKey && hit)
            toggleInSelection(hit.id); // desktop equivalent of the hold
        else if (e.shiftKey) clearSelection(p.x, p.y);
        else if (hit && selection.has(hit.id))
            designated = hit.id; // re-aim within the group
        else if (hit) {
            selection.clear();
...
function blowUp(k, born) {
    ...
    for (let i = 0; i < body.length; i++) {
        const a = world(body[i]),
            b = world(body[(i + 1) % body.length]);
        const mx = (a[0] + b[0]) / 2,
            my = (a[1] + b[1]) / 2;
        ...
        pieces.push({
            ...
            ay: a[1] - my,
            bx: b[0] - mx,
            by: b[1] - my, // about its own middle
            x: mx,
            y: my,
    ...
    for (let i = 0; i < 16; i++) {
        ...
        sparks.push({
            x: k.x,
            y: k.y,
            vx: (Math.cos(a) * speed) / slow,
            vy: (Math.sin(a) * speed) / slow,
            life: rnd(0.45, 1) * SPARK_MS * slow,
            hot: Math.random() < 0.5,
            ore: !!art.oreSparks,
        });
    }
...
function drawDebris(now) {
    ...
    for (const d of debris) {
        ...
        for (const k of d.sparks) {
            ...
            const kx = k.x + k.vx * (age / 1000) - cam.x,
                ky = k.y + k.vy * (age / 1000) - cam.y;
            const dot = new Path2D();
            dot.arc(kx, ky, (1.6 + 1.4 * f) / cam.zoom, 0, Math.PI * 2);
            ctx.fillStyle =
                k.ore ? `rgba(216,168,81,${f.toFixed(3)})`
                : k.hot ? `rgba(255,214,92,${f.toFixed(3)})`
                : `rgba(255,86,64,${f.toFixed(3)})`;
            ctx.fill(dot);
        }
        ctx.restore();
    }
...
const MODULE_ART = {
    gun: [
        // a crosshair: the thing that aims
    ...
    tractor: [
        ...
        'M16 8a6 6 0 0 1 0 8',
        'M8 16a6 6 0 0 1 0-8',
        'M19.5 4.5a11 11 0 0 1 0 15',
        'M4.5 19.5a11 11 0 0 1 0-15',
    ],
};
...
const TURRET_HP = 100,
    BAR_W = 22,
    BAR_H = 3,
    BAR_DROP = 13; // bar sizes are screen px
...
function wreckRing(x, y, frac) {
    ...
    if (frac > 0.001) {
        ...
        arc.arc(
            ...
            r,
            -Math.PI / 2,
            -Math.PI / 2 + Math.PI * 2 * Math.min(1, frac),
        );
...
// storing any of it.
const STAR_LAYERS = [
    { z: 0.35, tile: 900, per: 22, alpha: 0.3, size: 1.2 },
    { z: 0.6, tile: 1100, per: 16, alpha: 0.45, size: 1.5 },
    { z: 0.9, tile: 1300, per: 11, alpha: 0.65, size: 1.9 },
];
...
function tileStars(layer, li, tx, ty) {
    ...
    if (!pts) {
        ...
        rnd();
        rnd();
        pts = Array.from({ length: layer.per }, () => [
            rnd() * layer.tile,
            rnd() * layer.tile,
        ]);
...
function drawStars(cw, ch) {
    ...
    STAR_LAYERS.forEach((layer, li) => {
        const ox = cam.x * layer.z,
            oy = cam.y * layer.z; // parallax: nearer layers slide faster
        const size = layer.size / cam.zoom;
        ctx.fillStyle = `rgba(180,215,255,${layer.alpha})`;
        for (
            let tx = Math.floor((ox - halfW) / layer.tile);
            tx <= Math.floor((ox + halfW) / layer.tile);
            tx++
        )
            for (
                let ty = Math.floor((oy - halfH) / layer.tile);
                ty <= Math.floor((oy + halfH) / layer.tile);
                ty++
            )
...
function clearIcon(x, y, color) {
    ...
    icon.moveTo(x - arm, y - arm);
    icon.lineTo(x + arm, y + arm);
    icon.moveTo(x + arm, y - arm);
    icon.lineTo(x - arm, y + arm);
    ctx.save();
...
function drawPane() {
    for (const c of pane) {
        if (c.kind === 'rotate')
            rotateIcon(c.pos.x - cam.x, c.pos.y - cam.y, c.angle, rotating);
        else if (c.kind === 'clear')
            clearIcon(c.pos.x - cam.x, c.pos.y - cam.y);
        else if (c.kind === 'focus')
            clearIcon(c.pos.x - cam.x, c.pos.y - cam.y, FOCUS_COLOR);
    }
}
...
function drawSelection(s, isDesignated) {
    ...
    ctx.save();
    ctx.strokeStyle =
        isDesignated ? 'rgba(150,255,205,.9)' : 'rgba(95,240,176,.4)';
...
function drawControl(s) {
    ...
    ctx.save();
    ctx.strokeStyle =
        rotating ? 'rgba(95,240,176,.45)' : 'rgba(95,240,176,.20)';
...
const WALL_FILL = '#171f2b',
    WALL_EDGE = 'rgba(132,156,190,.85)';
...
const BLOCK_FILL = '#28323f',
    BLOCK_EDGE = 'rgba(176,196,222,.9)';

function drawWalls(vis) {
    const visible = [];
    for (const w of walls.values()) {
        if (w.x1 < vis.x0 || w.x0 > vis.x1 || w.y1 < vis.y0 || w.y0 > vis.y1)
            continue;
        visible.push(w);
    }
    ...
    for (const [mat, fill, edge] of [
        [undefined, WALL_FILL, WALL_EDGE],
        ['block', BLOCK_FILL, BLOCK_EDGE],
    ]) {
        const of = visible.filter(
            (w) => (w.mat === 'block') === (mat === 'block'),
        );
...
function draw() {
    ...
    if (dev)
        window.__rocks = driftList('rock', performance.now() - RENDER_DELAY);
    ...
    for (const id of [...selection])
        if (!fleet.some((s) => s.id === id)) selection.delete(id);
    if (!hasSelected && fleet.length) {
        // pick one on arrival, then leave it alone
        selection.add(fleet[0].id);
        designated = fleet[0].id;
        hasSelected = true;
    }
    ...
    const halfW = cw / 2 / cam.zoom + 120,
        halfH = ch / 2 / cam.zoom + 120;
    const vis = {
        x0: cam.x - halfW,
        x1: cam.x + halfW,
        y0: cam.y - halfH,
    ...
    for (const s of scenery.values()) {
        if (s.x1 < vis.x0 || s.x0 > vis.x1 || s.y1 < vis.y0 || s.y0 > vis.y1)
            continue;
        ctx.save();
        ctx.translate(-cam.x, -cam.y);
    ...
    for (const s of fleet) {
        ...
        const [sx, sy] = at(s);
        ctx.save();
        ctx.strokeStyle = 'rgba(95,240,176,.22)';
        ctx.lineWidth = 1 / cam.zoom;
        ctx.setLineDash([6 / cam.zoom, 8 / cam.zoom]);
        const tether = new Path2D();
        tether.moveTo(sx, sy);
        tether.lineTo(mx, my);
        ctx.stroke(tether);
        ctx.restore();
        ...
        poly(
            MARKER.map(([x, y]) => [x * pulse, y * pulse]),
            mx,
            my,
            now / 1400,
            '#5ff0b0',
    ...
    for (const s of state.ships) {
        ...
        for (const held of s.bm || []) {
            const grain =
                moving.ore.has(held) ?
                    stateAt(moving.ore.get(held), now - RENDER_DELAY)
                :   null;
            if (!grain) continue;
            ...
            const px = -dy / d,
                py = dx / d; // across the beam
            const near = BEAM_NEAR / cam.zoom,
                far = BEAM_FLARE / cam.zoom;
            const sx = s.x - cam.x,
                sy = s.y - cam.y,
                gx = grain.x - cam.x,
                gy = grain.y - cam.y;
            ...
            const pulse =
                0.5 + 0.5 * Math.sin((now / 1000) * BEAM_HZ * Math.PI * 2);
            ctx.save();
            ctx.fillStyle = `rgba(${BEAM_RGB},${0.1 + 0.2 * pulse})`;
            ctx.fill(beam);
            ctx.restore();
        }
    }
    ...
    drawGroup();
    for (const s of fleet)
        if (selection.has(s.id)) drawSelection(s, s.id === designated);
    for (const t of focusTargets()) reticle(t.x - cam.x, t.y - cam.y);
    if (holding) {
        const on =
            holding.ship !== null ?
                fleet.find((s) => s.id === holding.ship)
            :   holding;
    ...
    for (const s of state.ships) {
        ...
        if (art.draw) art.draw(x, y, now);
            ...
            for (const rib of art.ribs || [])
                poly(rib, x, y, s.a, color, false, 1);
        ...
        (s.ft || []).forEach((f, i) => {
            ...
            const gx = x + at[0] * cos - at[1] * sin,
                gy = y + at[0] * sin + at[1] * cos;
            if (hp <= 0) {
                // silenced: a mount, not a gun
                ...
                if (s.rp === i && !OFF.has('bars'))
                    wreckRing(gx, gy, (hp + wreckDepth) / wreckDepth);
                return;
            }
            if (art.guns && own && !OFF.has('arcs')) {
                // show each mount's traverse limits
                ctx.save();
                ctx.strokeStyle = 'rgba(95,240,176,.10)';
                ctx.lineWidth = 1 / cam.zoom;
                const arc = new Path2D();
                arc.arc(
                    gx,
            ...
            if (art.guns && aims(type))
                ...
                collar.arc(gx, gy, 6, 0, Math.PI * 2);
                ctx.stroke(collar);
                ctx.fillStyle = 'rgba(106,184,255,.9)';
                const eye = new Path2D();
                eye.arc(gx, gy, 3.4, 0, Math.PI * 2);
                ctx.fill(eye);
                ctx.restore();
            }
        ...
        if (!own && s.owner !== null && !OFF.has('labels')) {
            ...
            ctx.font = `${11 / cam.zoom}px ui-monospace, monospace`;
    ...
    const board = [...state.players]
        ...
        .map(
            (p) =>
                `${p.id === myId ? '>' : ' '} ${p.name.padEnd(8)} ${String(p.score).padStart(5)}`,
        )
        .join('\n');
    ...
    hud.textContent =
        ...
        (dev ?
            `  vib:${vib.ok ? 'api' : 'none'}/${vib.calls}/${vib.last}`
        :   '') +
        `\n` +
        (state.stale ?
            `** STALE: server.js changed on disk -- restart the server **\n`
        :   '') +
        `\n${board}`;
}
draw();
