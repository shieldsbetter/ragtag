...
(async()=>{
  const probe=new WebSocket("ws://localhost:8272"); let snap=null,pid=null;
  ...
  const t=(await get("http://localhost:9310/json")).find(x=>x.type==="page"&&x.url.includes("8272"));
  ...
  // find a screen point well away from both ships
  const empty=[80,H-80];
  const orders=()=>snap.ships.filter(s=>s.owner!==null&&s.owner!==pid&&s.dx!==undefined).length;
  const [A,B]=mine;
  await press(scr(A),60); await sleep(300);
  await press(scr(B),700); await sleep(400);     // both selected
  console.log(`  both selected; hold empty space to clear`);
  await press(empty,700); await sleep(500);       // long-press empty -> clear
  const before=orders();
  await press(empty,60); await sleep(900);        // tap empty -> should order nobody
  console.log(`  after clearing, a tap in space issued orders to: ${orders()-before} ships (want 0)`);
  await press(scr(A),60); await sleep(400);       // re-select one
  await press([W/2+120,H/2+120],60); await sleep(900);
  console.log(`  after re-selecting one, a tap ordered: ${orders()} ship(s) (want 1)`);
  process.exit(0);
